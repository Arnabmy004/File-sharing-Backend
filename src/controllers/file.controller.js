...
const getFile = asyncHandler(async (req, res) => {
  ...
  const downloadLink = `${baseUrl}/api/v1/file/download/id/${file._id}`;
  ...
  return res.status(201).json(
    new ApiResponse(
      200,
      { file, qrCode, downloadUrl: downloadLink, uniqueCode: code, expiresAt },
...
const getCode = asyncHandler(async (req, res) => {
  ...
  const baseUrl = getBaseUrl(req);
  const downloadLink = `${baseUrl}/api/v1/file/download/code/${code}`;
  const qrCode = await QRCode.toDataURL(downloadLink);
